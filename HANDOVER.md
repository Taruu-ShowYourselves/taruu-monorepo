...
3. **Deferred per-journey infra (STACK LOCKED — see [[taruu-stack-decisions]] / `DNS-SETUP.md`):** Solana, not qubik. **Remaining:** J5 = backing/swap via Jupiter (needs a wallet-connect decision); J6 = POD fulfilment webhook (needs Printful/Printify pick). **DONE this session:** ~~J4 OTP~~ (Workers KV, Twilio removed); ~~J7 refund~~ (request flow); ~~J9 NFT mint~~ (Solana cNFT via Bubblegum + Pinata + Helius, `/api/cron/mint-nfts` — needs mainnet smoke test + a merkle tree); ~~J7-push~~ (vote-results + new-city-vote push, `EXPO_ACCESS_TOKEN` wired).
