...
export const CONSENT_VERSION = 'doc-consent-v1-2026-07';
...
const COPY = {
  ...
  consentPoints: [
    ...
    'נשמרים: שם, תאריך לידה, תוקף התעודה, ואימות מוצפן של מספר הזהות.',
...
type Phase = 'consent' | 'details' | 'capture' | 'processing' | 'review' | 'submitting';
...
export function DocumentScanStep({
  ...
  /* ---- lifecycle: warm OCR once consented; release everything on unmount ---- */
  useEffect(() => {
    if (phase !== 'consent') preloadOcr();
  }, [phase]);
    ...
    <article className={styles.panel}>
      ...
      <div className={styles.actions}>
        <NewsButton
          ...
          trailing={<span aria-hidden>←</span>}
        >
          {submitting ? 'שולחים…' : 'אישור ושליחה'}
        </NewsButton>
        <button
          type="button"
          className={styles.linkButton}
